...
@TestPropertySource(properties = {
        "spring.config.location=classpath:routetest/kafka-sql-kafka/sql/error-select" + "/application.yml"})
@CamelSpringBootTest
@DirtiesContext
@Testcontainers
@AutoConfigureMetrics
public class KafkaSqlKafkaRouteOracleErrorSelectTest {
    ...
    private final static String MESSAGE_IN_KAFKA =
            getStringResources("routetest/kafka-sql-kafka/sql/error-select/test-message.json");
    ...
    @Test
    void verifyKafkaSqlKafkaOracleErrorSelect() throws Exception {
        int messageCount = 1;
