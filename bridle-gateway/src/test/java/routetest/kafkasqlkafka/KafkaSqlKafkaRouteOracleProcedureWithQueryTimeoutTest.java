...
@TestPropertySource(properties = {
        "spring.config.location=classpath:routetest/kafka-sql-kafka/procedure/query-timeout/application.yml"})
@CamelSpringBootTest
@DirtiesContext
@Testcontainers
@AutoConfigureMetrics
public class KafkaSqlKafkaRouteOracleProcedureWithQueryTimeoutTest {
    ...
    private final static String MESSAGE_IN_KAFKA =
            getStringResources("routetest/kafka-sql-kafka/procedure/query-timeout/test-message.json");


    @Container
    public static OracleContainer oracle =
            createOracleContainer().withCopyFileToContainer(MountableFile.forClasspathResource(
                                                                    "routetest/kafka-sql-kafka/procedure/query-timeout/init-oracle"),
    ...
    @Test
    void verifyKafkaSqlKafkaOracleProcedureStopsByReadTimeout() throws Exception {
        int messageCount = 1;
