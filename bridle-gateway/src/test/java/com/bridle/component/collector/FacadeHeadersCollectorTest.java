...
class FacadeHeadersCollectorTest {
    ...
    void processPutResultValueCollectorToCamelMessageHeaders() {
        ...
        when(collectorFactory.createValuesCollector(eq(ExpressionFormat.JSON), any())).thenReturn(mockValuesCollector);
        Map<String, Object> valuesByName = createValuesByName();
        Exchange mockExchange = mock(Exchange.class);
        Message mockMessage = mock(Message.class);
        when(mockExchange.getIn()).thenReturn(mockMessage);
        when(mockValuesCollector.collectValues(any())).thenReturn(Optional.of(valuesByName));
        ...
        assertEquals(valuesByName.size(), invocationsOfSetHeaderMethod.size());
    ...
    private void verifyThatAllValuesFromCollectorAnswerInMessageHeaders(Map<String, Object> valuesByName,
        ...
        invocationsOfSetHeaderMethod.forEach(invocation -> {
            ...
            assertTrue(valuesByName.containsKey(headerName));
            assertEquals(valuesByName.get(headerName), value);
        });
    }
